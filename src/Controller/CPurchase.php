...
class CPurchase {
    ...
    public static function addToCart($productId)
    {
        if (!(isset($_COOKIE['cart']))) {
            setcookie('cart', 0, time() + (300), "/"); // 5 minuti
        }
        ...
        $quantity_max = $found_product->getAvQuantity();
        if($cart[$productId] > $quantity_max) {
            $cart[$productId] = $quantity_max;
    ...
    */
    ...
    public static function addToCart($productId)
    {
        // Controlla se il cookie del carrello esiste, altrimenti crealo
        if (!isset($_COOKIE['cart'])) {
            setcookie('cart', json_encode([]), time() + (86400 * 30), "/"); // 30 giorni
            //var_dump($_COOKIE['cart']);
        }

        // Determina la quantità da aggiungere
        $quantity = ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['quantity'])) ? (int)$_POST['quantity'] : 1;

        // Decodifica il carrello dal cookie
        ...
        if (isset($cart[$productId])) {
            ...
            $cart[$productId] = min($quantity, $quantity_max);
        }

        // Salva il carrello aggiornato nel cookie
        setcookie('cart', json_encode($cart), time() + (86400 * 30), "/"); // 30 giorni
    ...
    */
    ...
    public static function completeOrder() {
        ...
        try {
            ...
            $cardNumber = isset($_POST['creditCard']) ? $_POST['creditCard'] : null;
    
            // Log dei dati ricevuti
            error_log("Valore di \$_POST['shipping']: " . print_r($shipping, true));
            error_log("Valore di \$_POST['creditCard']: " . print_r($cardNumber, true));
